...
describe('App', () => {
  ...
  it('switches all homepage copy between Chinese and English', () => {
    ...
    expect(screen.getByText(/工业级搜推广业务算法/)).toBeInTheDocument();
    ...
    expect(screen.getByText(/Industrial RecSys & Search pipelines/)).toBeInTheDocument();
  ...
  it('shows local-only draft notes in development mode', async () => {
    ...
    expect(await screen.findByRole('button', { name: /LLM八股 Overview/i })).toBeInTheDocument();
  ...
  it('opens the ML Coding section and displays ML Basics note', async () => {
    ...
    expect(screen.getByRole('button', { name: /00 · 数据预处理/i })).toBeInTheDocument();

    fireEvent.click(screen.getByRole('button', { name: /00 · 数据预处理/i }));
    expect(await screen.findByRole('heading', { name: /00 · 数据预处理与经典损失函数/i })).toBeInTheDocument();

    // Switch to English and verify translation
    fireEvent.click(screen.getByRole('button', { name: 'English' }));
    expect(await screen.findByRole('heading', { name: /00 · Data Preprocessing/i })).toBeInTheDocument();
  });
  ...
  it('routes directly to ML Coding 00B and ML Coding 01B via URL hash and renders them properly', async () => {
    ...
    expect(await screen.findByRole('heading', { name: /00B · 架构选型与混合精度训练/i })).toBeInTheDocument();

    fireEvent.click(screen.getByRole('button', { name: 'English' }));
    expect(await screen.findByRole('heading', { name: /00B · Architecture Decisions/i })).toBeInTheDocument();
    ...
    expect(await screen.findByRole('heading', { name: /01B · Transformer 架构变体/i })).toBeInTheDocument();
  });
  ...
  it('routes directly to ML Coding 06B (RLHF) and ML Coding 06C (RLVR / GRPO) via URL hash', async () => {
    ...
    expect(await screen.findByRole('heading', { name: /06B · RLHF 与偏好对齐全景/i })).toBeInTheDocument();

    fireEvent.click(screen.getByRole('button', { name: 'English' }));
    expect(await screen.findByRole('heading', { name: /06B · RLHF & Preference Alignment/i })).toBeInTheDocument();
    ...
    expect(await screen.findByRole('heading', { name: /06C · RLVR、推理模型与 Agentic RL/i })).toBeInTheDocument();
  ...
  it('routes directly to unified MLCoding07 Industrial ML Systems note', async () => {
    ...
    expect(await screen.findByRole('heading', { name: /07 · 推荐精排、重排与 A\/B 实验实战/i })).toBeInTheDocument();
    fireEvent.click(screen.getByRole('button', { name: 'English' }));
    expect(await screen.findByRole('heading', { name: /07 · Industrial RecSys/i })).toBeInTheDocument();
  });
