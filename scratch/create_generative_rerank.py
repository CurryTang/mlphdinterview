...
zh_content = """# 电商生成式重排全景：从候选序列化、Listwise 目标到时延与上线评估
...
- **位置衰减逆倾向得分（Inverse Propensity Scoring, IPS）**：
  历史数据中排在第 1 位的商品天然具有极高的曝光与点击优势。在计算 Listwise 交叉熵时，对位置 $k$ 的商品点击引入倾向倒数加权 $w_k = \frac{1}{P(\text{Examine} \mid \text{pos}=k)}$，消除由于展示位置带来的假阳性监督信号。
...
with open("notes/BusinessAlgorithm/BusinessAlgorithm10 Generative Reranking E-Commerce.md", "w", encoding="utf-8") as f:
    f.write(zh_content)
print("Successfully generated Chinese Generative Rerank note")
