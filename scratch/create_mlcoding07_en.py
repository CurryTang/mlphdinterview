...
en_content = """# ML Coding 07 · Industrial Recommendation Ranking: Base Model Family, Sampling, GAUC & Multi-Objective Seesaw Mitigation
...
$$\text{GAUC} = \frac{\sum_{g \in \mathcal{G}, \, n_g^+ > 0, \, n_g^- > 0} w_g \cdot \text{AUC}_g}{\sum_{g \in \mathcal{G}, \, n_g^+ > 0, \, n_g^- > 0} w_g}$$
...
- **LogLoss**: Requires importance weighting ($\frac{1}{w}$ for negative samples) for unbiased evaluation.

### 4. Ranking vs. Calibration
- **Ranking (Discrimination)**: Invariant to strictly monotonic transformations;
- **Calibration (Reliability)**: Evaluated via **PCOC** ($\frac{\sum \hat{p}_i}{\sum y_i} = 1.0$), **ECE**, and **Brier Score**.
...
- **Uncertainty Weighting**: Dynamic loss weighting $\mathcal{L} = \sum \frac{1}{2\sigma_k^2}\mathcal{L}_k + \ln \sigma_k$;
...
with open("notes/MLCoding/MLCoding07 Industrial Recommendation Ranking Metrics Sampling MultiObjective.en.md", "w", encoding="utf-8") as f:
    f.write(en_content)
print("Successfully created MLCoding07 English note")
