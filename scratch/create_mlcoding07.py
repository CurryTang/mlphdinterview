...
zh_content = """# ML Coding 07 · 工业级推荐排序全景：模型体系、负采样概率还原、GAUC 指标与多目标跷跷板治理
...
- **似然与损失指标（LogLoss）的重要性加权无偏估计（Importance Weighting）**：
  相比之下，交叉熵损失（LogLoss）强烈依赖正负样本比例。若要在采样数据上得到全量分布的无偏损失估计，必须使用重要性权重（Importance Weight $\frac{1}{w}$ 为负样本加权）：
...
with open("notes/MLCoding/MLCoding07 Industrial Recommendation Ranking Metrics Sampling MultiObjective.md", "w", encoding="utf-8") as f:
    f.write(zh_content)
print("Successfully created MLCoding07 Chinese note")
