...
| **Personalized Ranking** | **Group AUC (GAUC)** | Computes $\text{AUC}_u$ for each individual user $u$, weighted by impression count $w_u$:<br>$$\text{GAUC} = \frac{\sum_u w_u \cdot \text{AUC}_u}{\sum_u w_u}$$ | **The Core Offline North Star for Ranking**! Strips away inter-user activity bias and purely evaluates within-list personalized ranking quality. |
| **Probability Quality** | **LogLoss (Cross-Entropy)** | $-\frac{1}{N}\sum [y \log \hat{p} + (1-y)\log(1-\hat{p})]$. | Highly sensitive to absolute predicted probabilities; directly impacted by negative sampling. |
| **Position Sensitivity**| **NDCG@K / MRR** | Evaluates top-ranked items using logarithmic position discounts $\frac{1}{\log_2(\text{rank}+1)}$. | Aligns with user viewing patterns restricted to top viewports (Top 3 / Top 5). |
...
$$\text{PCOC} = \frac{\sum_{i=1}^N \hat{p}_i}{\sum_{i=1}^N y_i} = \frac{\text{Average Model Predicted Probability}}{\text{Average Observed Empirical CTR}}$$
...
with open("notes/BusinessAlgorithm/BusinessAlgorithm09 Industrial Recommendation Ranking Metrics Sampling.en.md", "w", encoding="utf-8") as f:
    f.write(en_content)
print("Successfully generated English note")
