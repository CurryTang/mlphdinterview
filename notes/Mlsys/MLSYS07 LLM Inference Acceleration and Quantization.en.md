# 07 · LLM Inference Acceleration, Speculative Decoding & Quantization

The autoregressive decoding phase exhibits exceptionally low arithmetic intensity ($O(1)$ FLOPs/Byte), causing compute units to idle while waiting for memory transfers. Accelerating LLM inference requires two complementary paradigms: **reducing the bytes transferred per token (low-bit quantization)** and **proposing and verifying multiple tokens in a single forward pass (speculative decoding)**. This chapter provides a rigorous deep dive into quantization (PTQ vs QAT, SmoothQuant, AWQ, GPTQ, FP8) and modern speculative decoding architectures (rejection sampling proofs, Medusa, EAGLE-3, and DFlash).

---

## Part 1: Inference Bottlenecks & Speculative Decoding Theory
...
## Part 2: Low-Bit Quantization Methods & Systems Engineering
...
### Module A: Speculative Decoding & Verification
## XII. Exercises
...
### Module B: Model Quantization & Precision Engineering

<details class="exercise">
<summary><span class="q-label">Exercise 1</span> <span class="q-text">What is the difference between symmetric and asymmetric quantization?</span></summary>

Symmetric quantization maps the zero-point to zero ($x_q = \text{round}(x/s)$), simplifying multiply-accumulate operations because zero-offset terms drop out. Asymmetric quantization adds an explicit zero-point offset ($z$), providing higher dynamic range for skewed activation distributions (e.g. ReLU/GELU outputs) at the cost of additional addition terms during GEMM.

</details>

<details class="exercise">
<summary><span class="q-label">Exercise 2</span> <span class="q-text">How does SmoothQuant resolve activation outliers without modifying model weights during inference?</span></summary>

SmoothQuant observes that activation outliers concentrate in specific feature channels. By applying a per-channel scaling factor $s$, activations are divided by $s$ while weights are multiplied by $s$. This mathematically equivalent transformation migrates quantization difficulty from activations to weights.
