...
这篇只讨论 2025 年之后真正影响长上下文系统设计的 attention 变体。老的 linear attention、Longformer、BigBird、普通 sliding window 不单独展开，除非它们能解释新方法的实现。
...
这篇理论论文里还有一个对工程很有用的结论：softmax 的 exponential kernel 通常有更好的 retrieval signal-to-noise ratio。直觉上，softmax 可以把少数相似 key 的权重拉得很尖；线性记忆把很多 `(key, value)` 外积加进同一个 `S_t`，读的时候更容易混入其他记忆的残差。也就是说，linear/delta attention 的优势主要是状态大小和 decode 成本，不是免费获得无限容量。
...
读 2025 之后的 efficient attention，建议一直问三个问题：
...
- 主 attention 仍然是 softmax retrieval，只是输入从“所有历史 token”变成“top-k token”。DeepSeek-V3.2 report 里的 sparse setting 使用每个 query 选择 2048 个 KV token，这个数字很适合面试里用来说明“不是小窗口，而是内容驱动的大预算稀疏检索”。
...
所以系统里会多出一类状态：`top-k indices`。GLM-5.2 的 IndexShare / IndexCache 就是在这里优化，后面的 KV cache 课会展开它如何在 MTP step 之间复用 indexer 选择结果。
...
这里 `v` 提供内容特征，`\Delta` 和 `A` 提供位置相关的参数化偏置，`top-w` 决定每个 query 真正保留的稀疏连接。这个公式不用死背，重点是它把“内容”和“位置”同时放进 mask 生成器，而不是只靠局部窗口。
...
DHSA 继续把选择做成层次化，但要注意它在论文实验里主要优化 prefill，decode 仍使用普通 dense attention；作者也把 decode sparse 化留作后续方向。它不是 DSA 那种每个 decode query 都先 top-k 的在线选择器。
...
FlashKDA 这类后端通常会有硬约束，例如 bf16、固定 K/V 维度、不支持部分 GVA 场景、不支持 context parallel。面试里如果只说“KDA 是 O(1) cache”，会显得太浅。更准确的说法是：
...
report 里的系统数字也值得记：MiniMax-M1 是 456B total、约 45.9B activated 的 hybrid MoE，支持 1M input context，M1-80k 版本最大输出 80K token。它的卖点不是“attention 名字更新”，而是在 100K 级生成长度下用更少 FLOPs 做 long reasoning rollout。
...
MiniMax-M1 的系统经验也很值得记。长输出 RL 训练时，他们发现 training kernel 和 inference kernel 的数值精度不一致会放大成 rollout 质量问题，最后通过提高 LM output head 精度到 FP32 修复，训练和推理 logprob 的相关性才恢复。这说明 efficient attention 不是 isolated layer choice，训练和推理 kernel 的数值一致性也会影响 RL。
