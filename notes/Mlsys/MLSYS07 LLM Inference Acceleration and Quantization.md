...
自回归解码阶段具有极低的算术强度（$O(1)$ FLOPs/Byte），导致计算单元在等待显存数据传输时严重空闲。加速推理的根本路径有二：**降低每个 Token 搬运的字节数（低比特量化）**与**单次前向输出验证多个 Token（投机采样）**。本章深度解析量化理论体系（PTQ vs QAT、SmoothQuant、AWQ、GPTQ、FP8 训练与推理）与投机采样前沿架构（拒绝采样分布等价性证明、Medusa、EAGLE-3 与 DFlash 扩散投机解码）。

---

## 第一部分：推理解码瓶颈与投机采样理论全景
