...
flowchart TB
    subgraph WQ[Work Queue]
      P1[Producer] --> Q1[Task Queue]
      Q1 --> A1[Worker A]
      Q1 --> B1[Worker B]
    end

    subgraph PS[Publish / Subscribe]
      P2[Publisher] --> T2[Topic / Exchange]
      T2 --> A2[Fraud subscriber]
      T2 --> B2[Analytics subscriber]
      T2 --> C2[Notification subscriber]
    end

    subgraph LOG[Partitioned Log]
      P3[Producers] --> L3[(Partition 0 · 1 · 2)]
      L3 --> G1[Consumer group A]
      L3 --> G2[Consumer group B]
      L3 --> R3[Replay from old offset]
    end
```

### 3.1 Work Queue

多个 worker 争抢同一批任务，一条任务通常只由其中一个 worker 完成。

适合图片处理、邮件发送、报表生成、模型离线推理。扩容方式很直接：增加 worker，同时观察积压是否下降。

### 3.2 Publish / Subscribe

一个业务事件被复制到多个逻辑订阅者。订单完成后，风控、积分、通知和数据分析可以各自处理，订单服务不需要知道这些下游的实现。

解耦依靠稳定的事件契约。架构图里多画一个 broker 并不会自动解耦：

- Producer 只发布稳定的事件 schema。
- Subscriber 自己管理处理进度和失败策略。
- 新增 subscriber 不要求修改 producer 的业务代码。

### 3.3 Partitioned Log

消息追加到有序 partition，消费方保存自己的 offset。消息不会因为某个消费者读过就立即删除，而是按 retention 保留，所以其他 consumer group 可以独立读取，也可以回到旧 offset 重放。

这类模型适合 CDC、用户行为流、审计事件和流式计算。代价是顺序、分区数、offset 提交和状态恢复都需要明确设计。

### 一张表记住差别

| 问题 | Work Queue | Pub/Sub | Partitioned Log |
|---|---|---|---|
| 一条消息给谁 | 一个 worker | 每个订阅者一份 | 每个 consumer group 一份 |
| 消费后是否保留 | 通常 ack 后删除 | 取决于订阅队列 | 按 retention 保留 |
| 重放历史 | 通常不擅长 | 依产品而定 | 核心能力 |
| 扩并行度 | 增加 worker | 每个订阅独立扩容 | 增加 partition 和 consumer |
| 典型用途 | 后台任务 | 业务事件广播 | 数据管道、CDC、stream processing |
...
3. 选择模型
   Work Queue、Pub/Sub、Partitioned Log，还是 DB queue？
