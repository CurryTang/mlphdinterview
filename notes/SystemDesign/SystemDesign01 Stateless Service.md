# System Design 01 · Stateless Service：从 Web Replica 到 LLM Serving
...
1. [[#一、背景：哪些地方会用 stateless，为什么要用]]
2. [[#二、先定义：什么叫服务进程不拥有状态]]
3. [[#三、传统 Web 服务怎么具体做成 stateless]]
4. [[#四、开发实现：API、DB、Redis、Queue 怎么配合]]
5. [[#五、部署实现：LB、health check、graceful shutdown、autoscaling]]
6. [[#六、代价：状态没有消失，只是被移动了]]
7. [[#七、LLM serving：为什么它更难]]
8. [[#八、LLM 里的具体解决方案]]
9. [[#九、Agent / RL 系统里的 stateless-ish worker]]
10. [[#十、面试回答模板]]
11. [[#十一、练习题]]

---

## 一、背景：哪些地方会用 stateless，为什么要用

Stateless service 是 system design 里最早出现的基础概念之一，因为它决定了一个系统能不能横向扩容、滚动发布、故障替换和自动恢复。只要你看到“多个 service replica 接同一类请求”，基本都会遇到这个问题：
...
| 自动扩缩容 | replica 可以随时增加或减少，系统状态不依赖某个固定进程 |

所以 stateless 的主要目的不是“架构更优雅”，而是解决四个很实际的问题：

```text
Scale out:       流量上来时可以复制更多 replica
Fault tolerance: 单个实例挂掉不损坏系统状态
Deployability:   可以滚动更新，不需要停机
Operability:     autoscaling、health check、drain、retry 才有意义
```
...
这也是本讲要建立的核心直觉：先理解传统 Web 怎么做 stateless，再看 LLM serving 为什么只能做到 stateless-ish。
...
## 四、开发实现：API、DB、Redis、Queue 怎么配合
...
### 7.1 方案一：每次带完整 conversation，任意 worker 重新 prefill
...
### 7.2 方案二：sticky routing，把同一个 session 路由到同一个 worker
...
### 7.3 方案三：conversation durable，KV cache best-effort
...
### 7.4 方案四：prefix cache，把共享前缀当成可复用状态
...
### 7.5 方案五：paged KV cache，让 GPU 内存状态可管理
...
### 7.6 方案六：disaggregated prefill / decode
...
## 九、Agent / RL 系统里的 stateless-ish worker
...
```quiz
title: Stateless Service · Check 1
...
```quiz
title: Stateless Service · Check 2
...
```quiz
title: Stateless Service · Check 3
...
```quiz
title: Stateless Service · Check 4
