...
1. [[#一、先定义：什么叫服务进程不拥有状态]]
2. [[#二、传统 Web 服务怎么具体做成 stateless]]
3. [[#三、开发实现：API、DB、Redis、Queue 怎么配合]]
4. [[#四、部署实现：LB、health check、graceful shutdown、autoscaling]]
5. [[#五、代价：状态没有消失，只是被移动了]]
6. [[#六、LLM serving：为什么它更难]]
7. [[#七、LLM 里的具体解决方案]]
8. [[#八、Agent / RL 系统里的 stateless-ish worker]]
9. [[#九、面试回答模板]]
10. [[#十、练习题]]
...
## 三、开发实现：API、DB、Redis、Queue 怎么配合
...
## 四、部署实现：LB、health check、graceful shutdown、autoscaling
...
## 八、Agent / RL 系统里的 stateless-ish worker
