...
### 2. 指数分布 $\mathrm{Exp}(\lambda)$ 的最大似然估计
...
### 3. 均匀分布 $\mathrm{Unif}[0,\theta]$ 的最大似然估计：偏差、方差与无偏修正
...
### 4. $N(0,1)$ 对 $N(\mu,1)$ 的似然比统计量与充分统计量的识别
...
### 5. $f(x\mid\theta)=\dfrac{2x}{\theta^2}$（$0\le x\le\theta$）的矩估计
...
### 6. 比较 MLE 与矩估计：以 $f(x\mid\theta)=\dfrac{2x}{\theta^2}$ 为例

**思路**：模块四已经得到这个密度的矩估计 $\hat\theta_C$。现在对同一个密度重复问题 3 的边界论证求最大似然估计 $\hat\theta_A$，再把两个估计量的偏差、方差、均方误差放在一起比较。
...
和问题 3 一样，这在合法定义域 $\theta\ge X_{(n)}$ 上是 $\theta$ 的严格递减函数，最大值在定义域左端点取到：
