...
一个 $n$ 人有限标准式博弈定义为一个三元组 $\mathcal{G} = \left( \mathcal{N}, (S_i)_{i \in \mathcal{N}}, (u_i)_{i \in \mathcal{N}} \r\right)$：
...
2. **第 2 轮剔除**：已知所有理性人的出价均落入 $[0, 66.67]$，则全场均值 $\mu \le 66.67$，目标值上限变为 $\frac{2}{3} \times 66.67 = \frac{400}{9} \approx 44.44$。
   - 策略空间收缩为 $S_2 = [0, 44.44]$。
3. **第 $k$ 轮递归收缩**：
   $$S_k = \left[ 0, 100 \times \left( \frac{2}{3} \r\right)^k \r\right]$$
4. **求极限**：
   $$\lim_{k \to \infty} 100 \times \left( \frac{2}{3} \r\right)^k = 0 \implies S_\infty = \{0\}$$
...
- 设对称单调递增出价函数 $b(v)$，反函数 $\beta(b)$。买家期望收益：
  $$\mathbb{E}[\Pi(b \mid v)] = (v - b) \cdot \mathbb{P}\left(\max_{j \ne i} v_j < \beta(b) \r\right) = (v - b) \cdot [\beta(b)]^{n-1}$$
- 对出价 $b$ 求一阶偏导（FOC）：
...
- 两边同乘积分因子 $v^{n-2}$：
  $$\frac{d}{dv} \left[ b(v) v^{n-1} \r\right] = (n - 1) v^{n-1} \implies b(v) v^{n-1} = \frac{n - 1}{n} v^n + C$$
...
- **严密条件期望推导**：
  ...
  - 公司 A 的期望净利润为：
    $$\mathbb{E}[\text{Profit}] = \mathbb{P}(V \le B) \cdot \left( \mathbb{E}[1.5 V \mid V \le B] - B \r\right) = \frac{B}{100} \cdot (0.75 B - B) = -\frac{0.25 B^2}{100} \le 0$$
