# Quant 13 · 博弈论与策略性决策：纳什均衡、极小极大定理与动态规划（Game Theory & Strategic Decision Making）

博弈论（Game Theory）是分析多主体策略交互、逆向归纳与信息不对称决策的数学理论。在做市商报价、拍卖机制设计、暗池流动性对抗与算法执行中，博弈论构成了微观对抗建模的理论基石。

本文系统梳理非合作博弈与组合博弈的核心理论，提炼出 **5 大核心分析框架（Core Analytical Frameworks）**，并对《绿皮书》（*A Practical Guide To Quantitative Finance Interviews*）与《街头传闻》（*Heard on the Street*）中的 **10 大经典博弈模型** 进行严格的数学推导与通用参数化建模。
...
> **核心定理（无差异原理）**：在混合策略纳什均衡中，局中人 $i$ 策略支撑集（Support, 即被赋予正概率的所有纯策略 $\operatorname{supp}(\sigma_i)$）中的**每一个纯策略所带来的期望收益必须严格相等**，且不低于未被使用的纯策略收益：
> 
...
在两人零和博弈（$u_1(s_1, s_2) + u_2(s_1, s_2) = 0$）中，局中人 1 最大化收益等价于局中人 2 极小化局中人 1 的收益。设局中人 1 收益为 $u(s_1, s_2)$，则冯·诺依曼极小极大定理保证：
...
#### 极小化极大与无差异原理的深层对偶机理（“削峰填谷”与期望抹平）

为什么在混合策略纳什均衡中，对手各备选动作的期望得分必须被精确抹平？

**核心机制剖析**：
核心原因在于博弈论中的**“极小化极大（Minimax）定理”**：如果不同选项的期望得分不相等，B 就会针对性地选择期望最高的那一个；A 为了把 B 的最大得分压到最低，就必须“削峰填谷”，直到所有选项被彻底抹平。

具体数学逻辑如下：
设局中人 A 采用混合策略 $p$。面对理性对手 B 的 $K$ 个可选纯策略选项 $\{s_{B,1}, s_{B,2}, \dots, s_{B,K}\}$，在 A 的策略 $p$ 下，各个选项带给对手 B 的期望得分分别为：
$$E_B(s_{B,1}; p), \; E_B(s_{B,2}; p), \; \dots, \; E_B(s_{B,K}; p)$$

1. **对手的定点收割倾向**：
   对手 B 作为一个理性的收益最大化者，在观测或推断出 A 的混合概率分布 $p$ 后，**必然会针对性地选择期望得分最高的那一个选项**：
   $$\text{Best Response of B} = \arg\max_{j} E_B(s_{B,j}; p)$$
   此时 B 的实际得分即为其各选项的上界：$\max_j E_B(s_{B,j}; p)$。

2. **A 的“削峰填谷”必然性**：
   局中人 A 的目标是将对手 B 的最大得分压到最低（即求解 $\min_p \max_j E_B(s_{B,j}; p)$）：
   - 如果不同选项的期望得分不相等（例如选项 1 期望为 2，选项 2 期望为 5），B 就会定点选择选项 2，获得高分 5；
   - 此时存在一个“收益峰值”。A 为了把 B 的得分压低，就必须调整自身动作的概率分布，将概率权重向抑制选项 2 的方向倾斜；
   - 这种调整会导致高期望项下降、低期望项上升，呈现出**“削峰填谷”**的平抑动态；
   - 只要各选项期望仍不相等，B 就始终能抓住最高峰进行攻击，A 就始终有动力进一步平抑峰值；
   - **最终稳态**：A 必须持续削峰填谷，**直到对手 B 支撑集内所有选项的期望得分被彻底抹平**：
     $$E_B(s_{B,1}; p^*) = E_B(s_{B,2}; p^*) = \dots = E_B(s_{B,K}; p^*) = V^*$$

3. **几何图景（凸包上包络线极小化）**：
   函数 $M(p) = \max_j E_B(s_{B,j}; p)$ 是若干个关于 $p$ 的仿射线性函数的**上包络线（Upper Envelope）**，构成一个分段线性的下凸函数（Convex V-shape）。
   由于各条直线斜率正负相反，上包络线的最低点（极小值点 $\min_p M(p)$）恰好落在各直线相互交叉的**顶点交点处（Intersection）**。在交点处，两项或多项期望收益严格相等。因此，**极小化极大的最优化条件，在几何上必然等价于使对手候选选项的期望收益达到完全无差异**。
...
## 模块二：博弈论核心分析框架（Core Analytical Frameworks）

根据博弈的信息结构与行动时序，可将经典博弈模型归纳为 5 大核心分析框架：

| 分析框架 | 适用博弈特征 | 核心数学机制 | 典型应用模型 |
| :--- | :--- | :--- | :--- |
| **1. 逆向归纳与奇偶递归** | 完全信息、有限阶段动态博弈 | 子博弈精炼纳什均衡（SPE）、Kuhn 定理、倒序递推 | 海盗分金、老虎吃羊、三方决斗 |
| **2. 严格劣势策略迭代剔除** | 完全信息、策略空间连续/可测 | 收缩映射（Contraction Mapping）、Banach 不动点 | 猜均值的 2/3、旅行者困境 |
| **3. 混合策略与 Minimax 削峰填谷** | 完全信息、离散对抗性静态博弈 | 冯·诺依曼极小极大定理、无差异原理、线性代数系统 | 点球对决、检查博弈、俄罗斯轮盘赌 |
| **4. 贝叶斯纳什与拍卖微分方程** | 不完全信息、私有估值连续出价 | 贝叶斯纳什均衡（BNE）、一阶条件 ODE、包络定理 | 一阶/二阶/全支付拍卖、胜者诅咒收购 |
| **5. 公平组合博弈与不变性论证** | 确定性二人离散完全信息博弈 | Sprague-Grundy 定理、Nim-Sum 异或和、偷策略反证法 | 取硬币博弈、Chomp 网格、囚犯帽子校验 |

---

### 框架 1：逆向归纳与奇偶性递归（Backward Induction & Parity）
...
### 框架 2：收缩映射与劣势策略迭代剔除（Contraction Mapping & IESDS）
...
### 框架 3：混合策略无差异原理与 Minimax 削峰填谷（Indifference Principle & Minimax）
- **适用场景**：石头剪刀布变体、检查博弈（Inspection Game）、扑克下注 Bluffing、俄罗斯轮盘赌。
- **标准三步法**：
  1. **设自身混合概率分布**：设局中人 A 以概率向量 $p = (p_1, \dots, p_m)$ 行动；
  2. **列出对手各动作的期望收益函数**：计算对手 B 选择纯策略 $j$ 时的得分 $E_B(j; p)$；
  3. **执行 Minimax 削峰填谷联立方程**：基于极小化极大原理，令对手所有在支撑集内的动作期望收益严格相等并彻底抹平：$E_B(1; p^*) = E_B(2; p^*) = \dots = V^*$，求解最优混合分布 $p^*$。

---

### 框架 4：贝叶斯对称出价与常微分方程（Bayesian Nash & Auction ODE）
...
### 框架 5：公平组合博弈、Nim 和与偷策略论证（Combinatorial Games & Strategy-Stealing）
...
### 2. 老虎与羊的岛屿博弈（Tigers and Sheep Island）

> **问题定义（老虎吃羊 / 奇偶不变量与诱因威胁）**：
...
#### 分析方法：奇偶归纳法（Parity Backward Induction）
...
### 3. 三方决斗博弈（The Truel: 3-Player Duel with Different Accuracies）

> **问题定义（三方决斗 / 状态转移概率与蓄意轮空）**：
...
### 5. 俄罗斯轮盘赌决策（条件概率与贝叶斯筛选）

> **问题定义（俄罗斯轮盘赌 / 贝叶斯筛选与条件胜率）**：
...
### 6. 经典拍卖理论全景（一阶、二阶、全支付与收益等价）

> **问题定义（拍卖理论 / 密封价格竞价与收益等价定理）**：
...
### 7. 胜者诅咒与未知价值公司收购（Winner's Curse in Corporate Acquisition）

> **问题定义（胜者诅咒 / 逆向选择与截断条件期望）**：
> 公司 A 打算收购目标公司 T。
> 规则：
...
### 8. 排成一行的取硬币博弈（Coins in a Line Game）

> **问题定义（硬币排线博弈 / 奇偶不变量与先手必不输）**：
...
### 9. Chomp 巧克力吃网格博弈与偷策略论证（Chomp & Strategy-Stealing）

> **问题定义（Chomp 吃网格博弈 / 偷策略反证法）**：
...
#### 分析方法：偷策略反证法（Strategy-Stealing Argument）
...
### 10. 100 个囚犯与红蓝帽子（100 Prisoners Hat Riddle & Parity Code）

> **问题定义（100 个囚犯红蓝帽子 / 奇偶校验广播策略）**：
...
| 经典博弈模型 | 核心博弈特征与类型 | 分析方法 / 归纳逻辑 | 最优决策 / 核心结论 |
| :--- | :--- | :--- | :--- |
| **海盗分金 ($N$ 人 $M$ 金币)** | 完全信息有限动态博弈 | 逆向归纳法 | 5 人分 100 金币方案：$(96, 0, 1, 0, 1)$ |
| **老虎与羊 ($N$ 老虎 1 羊)** | 诱因威胁与奇偶吸收博弈 | 奇偶归纳法 | 奇数只老虎吃，偶数只老虎绝不吃 |
| **三方决斗 (Truel: $1/3, 2/3, 1$)** | 离散 Markov 状态转移动态博弈 | 残局逆向推理 | 最弱者首轮**朝天放枪 (Pass)**，胜率高达 $39.7\%$ |
| **猜均值 $2/3$ (Beauty Contest)** | 连续协调博弈 / 选美博弈 | 收缩映射 IESDS | 唯一纳什均衡为所有人出 $0$ |
| **俄罗斯轮盘赌决策** | 不完全信息贝叶斯筛选 | 条件概率对比 | 相邻装填选**不重转** (25%)；不相邻选**必须重转** (33%) |
| **一阶价格密封拍卖 (FPA)** | 独立私有估值对称静态博弈 | 对称微分方程 | $b^*(v) = \frac{n-1}{n} v$（双人拍卖出价打五折） |
| **二阶价格密封拍卖 (SPA)** | 显式弱占优策略机制 | 占优策略验证 | $b^*(v) = v$（诚实申报真实估值） |
| **未知价值公司收购 (胜者诅咒)** | 逆向选择与条件期望博弈 | 截断均值条件期望 | $B^* = 0$（面临胜者诅咒，任何正出价必亏） |
| **排成一行的硬币博弈** | 二人确定零和组合博弈 | 奇偶位置不变量 | 先手选择较大奇偶组，保底获得 $\ge 50\%$ 面值 |
| **Chomp 巧克力吃网格** | 无平局完全信息无偏博弈 | 偷策略反证法 | 先手必定存在必胜策略 (Non-constructive) |
| **100 囚犯红蓝帽子** | 分布式无噪声协作博弈 | 奇偶校验广播码 | 尾人牺牲广播全局奇偶，前 99 人 $100\%$ 存活 |
