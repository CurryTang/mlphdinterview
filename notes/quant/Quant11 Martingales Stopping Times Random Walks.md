...
由于单步增量 $\Delta_n$ 的分子 $B - 3n$ 随 $n$ 严格单调递减，一旦 $n \ge \lfloor B/3 \rfloor$ 后，未来所有步的 $\Delta_k \le 0$ 恒成立。根据 **Chow-Robbins 单调停止定理（Monotone Stopping Theorem）**，单步前瞻准则（1-Step Look-Ahead）保证了局部最优解就是全局最优解！
