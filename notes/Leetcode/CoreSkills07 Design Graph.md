...
#### Kahn 算法：原理、算法流程与严密正确性证明

Kahn 算法由 Arthur B. Kahn 于 1962 年发表在 *Communications of the ACM* 的经典论文 *"Topological sorting of large networks"* 中提出，是解决**有向图拓扑排序（Topological Sort）**与**有向图判环（Directed Cycle Detection）**最高效、最直观的工业级算法。

#### 1. 形式化定义与物理直觉

对于有向图 $G = (V, E)$：
- **入度（In-degree）**：$\text{indegree}[v] = |\{u \in V \mid (u, v) \in E\}|$，表示节点 $v$ 拥有的**前置依赖数量**。
- **拓扑排序（Topological Order）**：顶点集 $V$ 的一个全排列 $(v_1, v_2, \dots, v_n)$，满足对图中任意有向边 $(u, v) \in E$，$u$ 在排列中都严格出现在 $v$ 之前。
- **物理直觉（剥洋葱与依赖消减）**：
  - $\text{indegree}[u] = 0$ 表示节点 $u$ **没有任何前置依赖**（或所有前置已全部满足），可以安全“执行”；
  - 执行 $u$ 后，将其从图中物理剥离，其所有下游后继 $v$ 的前置计数减 1（$\text{indegree}[v] \leftarrow \text{indegree}[v] - 1$）；
  - 若某个 $v$ 的入度降为 0，说明其依赖已全部清除，成为下一批可执行的“自由”节点。

#### 2. 算法标准流程

```text
1. 遍历图的所有边，计算每个节点的初始入度 indegree[v]。
2. 将所有入度为 0 的节点放入队列 queue（所有自由起点）。
3. 初始化拓扑序输出数组 order = []。
4. 当队列非空时循环：
   a. 弹出队头节点 u，将 u 加入 order；
   b. 遍历 u 的所有邻接后继节点 v（即存在边 u -> v）：
      - indegree[v] -= 1  (模拟删除边 u -> v)
      - 若 indegree[v] == 0，将 v 加入 queue；
5. 完备性检验：
   - 若 len(order) == |V|：成功生成合法拓扑序 order。
   - 若 len(order) < |V|：图中存在有向环，无法完成拓扑排序。
```
...
复杂度为 $\mathcal{O}(V + E)$，因为每个节点进出队列恰好 1 次，每条有向边也恰好被遍历并消减 1 次。

#### 3. 严密正确性数学证明（Mathematical Correctness Proof）

很多工程师只记住了 Kahn 算法的代码模板，却无法在面试深挖时回答：**“为什么 Kahn 算法一定能找到起点？为什么弹出的顺序一定不违背拓扑约束？为什么出队节点数不足时图里一定有环？”** 

以下三条定理构成了 Kahn 算法完整闭环的数学基石：

---

##### 定理一（起点存在性定理）：任何非空有限有向无环图（DAG）必定至少存在一个入度为 0 的节点

> **证明（鸽巢原理与反证法）**：
> 假设有限有向图 $G = (V, E)$（顶点数 $|V| = n \ge 1$）是 DAG，但**不存在**入度为 0 的节点。
> 也就是说，图中每个顶点的入度均满足 $\text{indegree}[v] \ge 1$。
> 
> 我们任选一个起点 $v_1$。由于 $\text{indegree}[v_1] \ge 1$，必存在前驱 $v_2 \in V$ 使得 $(v_2, v_1) \in E$；
> 同理，由于 $\text{indegree}[v_2] \ge 1$，必存在前驱 $v_3 \in V$ 使得 $(v_3, v_2) \in E$；
> 依此类推，我们可以沿着入边不断逆向寻找前驱，构造出一条无限长度的前驱序列：
> $$\dots \to v_{k+1} \to v_k \to \dots \to v_2 \to v_1$$
> 然而，图的顶点集合是有限的（只有 $n$ 个不同顶点）。根据**鸽巢原理（Pigeonhole Principle）**，序列逆行最多 $n$ 步后，必定会出现重复节点，即存在 $i < j$ 满足 $v_i = v_j$。
> 此时子序列 $v_i \to v_{i-1} \to \dots \to v_{j+1} \to v_j (= v_i)$ 构成了图中的一个**有向环**。
> 这与 $G$ 是有向无环图（DAG）的假设**严重矛盾**！
> 因此反设不成立：**任何有限 DAG 中必然至少存在一个入度为 0 的节点**（同理，也必然至少存在一个出度为 0 的节点）。$\blacksquare$

---

##### 定理二（拓扑序合法性定理）：若图 $G$ 是 DAG，Kahn 算法输出的序列是 $G$ 的合法拓扑排序

> **证明（强数学归纳法）**：
> 对图的节点规模 $n = |V|$ 进行归纳：
> 
> 1. **基础步（$n = 1$）**：
>    只有一个节点且无自环，$\text{indegree} = 0$，直接出队加入结果，排列显然满足拓扑序。
> 2. **归纳假设**：
>    假设对于任意节点数 $k < n$ 的 DAG，Kahn 算法均能正确输出合法拓扑排序。
> 3. **归纳步（规模 $n$）**：
>    由定理一，DAG 中必存在入度为 0 的节点 $u$。
>    - 由于 $\text{indegree}[u] = 0$，图 $G$ 中**不存在任何指向 $u$ 的有向边**（即不存在任何 $w$ 使得 $(w, u) \in E$）。因此，将 $u$ 放在拓扑序列的最前位置，**绝不可能违反任何一条边的先后次序约束**。
>    - 将 $u$ 及其所有出边 $(u, v)$ 从图 $G$ 中物理移除，得到剩余诱导子图 $G' = G \setminus \{u\}$。
>    - 易证：从 DAG 中删除节点和边后，剩余子图 $G'$ **依然是 DAG**（删除元素不可能凭空产生新的环）。
>    - 子图 $G'$ 的规模为 $n - 1$。由归纳假设，Kahn 算法对 $G'$ 执行后续操作，将产生 $G'$ 的合法拓扑序 $\sigma'$。
>    - 将 $u$ 置于 $\sigma'$ 之前，形成序列 $(u, \sigma')$。对于图 $G$ 中的任意边 $(x, y) \in E$：
>      - 若 $x = u$，则 $y \in \sigma'$，$u$ 排在 $y$ 前面，约束满足；
>      - 若 $x \neq u$，则边 $(x, y)$ 完全属于子图 $G'$，由归纳假设在 $\sigma'$ 中 $x$ 排在 $y$ 前面，约束满足。
>    - 因此，输出全排列严格满足原图 $G$ 的所有拓扑约束。$\blacksquare$

---

##### 定理三（环检测充要性定理）：有向图 $G$ 存在环 $\iff$ Kahn 算法输出节点数 $\text{len}(\text{order}) < |V|$

> **证明（充分必要性双向证明）**：
> 
> - **充分性（若图有环 $\implies \text{len}(\text{order}) < |V|$）**：
>   设图 $G$ 中存在有向环 $C = v_1 \to v_2 \to \dots \to v_k \to v_1$。
>   对于环上的每一个节点 $v_i$，它都至少拥有一条来自环内前驱的入边 $(v_{i-1}, v_i)$。
>   要使 $v_i$ 的入度降为 0，其前驱 $v_{i-1}$ 必须先被出队删除；而要使 $v_{i-1}$ 出队，其更前驱又必须先被删除。
>   这是一个闭合死锁环：**环上没有任何一个节点能够率先达到入度为 0**！
>   因此，环 $C$ 中的所有节点永远无法入队，也永远无法加入 `order`。最终输出长度 $\text{len}(\text{order}) \le |V| - k < |V|$。
> 
> - **必要性（若 $\text{len}(\text{order}) < |V| \implies$ 图中必有环）**：
>   假设算法终止（队列为空）时，未进入 `order` 的剩余节点集合为 $S = V \setminus \text{order}$，且 $S \neq \emptyset$。
>   因为此时队列已空，说明**当前集合 $S$ 中不存在任何入度为 0 的节点**。
>   换言之，对任意 $s \in S$，节点 $s$ 至少还有一条入边。
>   注意：所有来自已处理集合 $V \setminus S$ 的边在对应节点出队时已被完全消减，因此节点 $s$ 的未消减入边**必然完全来自于集合 $S$ 内部**。
>   这意味着在由 $S$ 诱导的子图 $G[S]$ 中，每个顶点的入度都满足 $\text{indegree}_{G[S]}[s] \ge 1$。
>   由定理一的反向推论（如果一个非空有限有向图的每个节点入度都 $\ge 1$，则该图必含环），子图 $G[S]$ 内**必然至少包含一个有向环**。$\blacksquare$

#### 4. Kahn 算法 vs. 三色标记 DFS 对比

| 比较维度 | Kahn 算法 (入度 BFS) | 三色标记法 (DFS 后序反转) |
|---|---|---|
| **推进方向** | **正向推进**：从源点（入度 0）逐步向汇点剥离 | **逆向回溯**：深入到汇点（出度 0），后序自底向上回溯 |
| **拓扑序生成** | 节点出队时**即刻追加**到结果末尾 | 节点所有后继探完后入栈/追加，最后**整体反转** |
| **环检测机制** | **全局计数死锁**：最终检查 $\text{len}(\text{order}) == \|V\|$ | **局部路径探查**：遍历中遇到处于栈内（状态 1 / visiting）的节点即发现回边（Back-Edge） |
| **工程并发性** | **天然分层并行**：同一批入度为 0 的节点完全独立，可由多线程并发执行（如构建系统 Makefile/Bazel 的并行编译） | 深度优先单线递归，不易直接进行同层无依赖任务的并发调度 |
| **空间与实现** | 需要显式维护入度数组 `indegree` 和队列，无爆栈风险 | 依赖调用栈（或显式栈），深度过大时需警惕栈溢出 |
