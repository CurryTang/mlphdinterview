...
#### 3. 为什么头插法好？直接套用双指针推进的三大失败用例 (Failure Cases)

若直接对局部区间套用 LC 206 的“指针同步前移”策略（`curr.next = prev; prev = curr; curr = nxt`），会直接引发以下三种典型的指针故障：

- **失败用例 1：静默数据丢失（Silent Truncation - 子链断开孤悬）**：
  若在进入区间时设 `prev = None` 并执行翻转：
  ```text
  初始状态: [1] -> [2] -> [3] -> [4] -> [5] -> None (反转 [2, 4])
  循环执行:
    节点 1 保持原本指向: [1] -> [2]
    但节点 2 已指向 None: [2] -> None  (主链由此截断！)
    反转子链成为孤立链: [4] -> [3] -> [2] -> None (无前驱连入)
    剩余后继节点丢失:   [5] -> None  (无前驱连入)
  结果: 若直接返回 head，只能读取到 [1, 2]，节点 3、4、5 发生静默数据丢失！
  ```

- **失败用例 2：死锁闭环（Deadly Cycle / Infinite Loop）**：
  若尝试将 `prev` 初始指向前驱节点 1（`prev = 1, curr = 2`），并在第 1 轮执行 `curr.next = prev`（即 `2.next = 1`）：
  ```text
  初始状态: [1] -> [2] -> [3] -> ...  (1.next 原本就指向 2)
  指针重指: 2.next = 1
  内存状态: [1] <===> [2]  (节点 1 与节点 2 双向死锁互指！)
  结果: 随后的任何遍历操作均会在此环形结构中无限死循环 (TLE / OOM)。
  ```

- **失败用例 3：四指针手工缝合极其脆弱（Stitching Fragility & NPE）**：
  若为了规避上述问题而强行使用双指针，必须人工追踪 4 个边界点：`p_before` (1)、`sub_tail` (2)、`sub_head` (4)、`p_after` (5)，并在翻转后手动缝合：
  `p_before.next = sub_head; sub_tail.next = p_after`。
  一旦遇到 `left = 1`，`p_before` 为 `None`，调用 `p_before.next` 立即抛出空指针异常，必须处处打 `if/else` 特判补丁。

**头插法的根本优势**：
头插法在**任何一次循环操作的内部与前后，整条链表在物理上均保持 100% 完整连通**。它从未将子链表“剪切”下来，而是像从牌堆中间抽牌一样插到前驱桩位之后，从数学结构上**天然免疫断链、天然免疫成环、天然免除二次拼接**。配合 `dummy = ListNode(0, head)`，即使 `left = 1` 也零特殊分支。
