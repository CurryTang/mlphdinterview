...
**Examples**:
- **Full Reversal (LC 206)**: `head = [1, 2, 3, 4, 5]` $\implies$ `[5, 4, 3, 2, 1]`
- **Subsegment Reversal (LC 92)**: `head = [1, 2, 3, 4, 5], left = 2, right = 4` $\implies$ `[1, 4, 3, 2, 5]`
- **Boundary Case**: `head = [3, 5], left = 1, right = 2` $\implies$ `[5, 3]`
...
class ReverseListSolution:
    ...
    def reverseListIterative(cls, head: Optional[ListNode]) -> Optional[ListNode]:
        """
        Reverses a singly linked list iteratively in-place (LC 206).
    ...
    def reverseListRecursive(cls, head: Optional[ListNode]) -> Optional[ListNode]:
        """
        Reverses a singly linked list recursively (LC 206).
    ...
    def reverseBetween(cls, head: Optional[ListNode], left: int, right: int) -> Optional[ListNode]:
        """
        Reverses a subsegment of a linked list in-place (LC 92) via Sentinel Dummy Head + Head Insertion.
        Time O(N), auxiliary space O(1).
        """
        dummy = ListNode(0, head)
        prev = dummy
        # 1. Step to the node immediately preceding the reversal subsegment
        for _ in range(left - 1):
            prev = prev.next

        curr = prev.next  # First node of the subsegment (acts as anchor, pushed to subsegment tail)

        # 2. Perform (right - left) head insertion operations
        for _ in range(right - left):
            nxt = curr.next        # Unlink: identify target node to be moved forward
            curr.next = nxt.next  # Bypass: bridge curr to the remainder of the list
            nxt.next = prev.next  # Prepend: splice nxt to the front of the reversed sublist
            prev.next = nxt       # Anchor update: rewire prev to point to new subsegment head
...
<div class="review-block">
<div class="review-block-label">💡 Mechanism & Invariants: Head Insertion Dynamics & Mental Models</div>

- **Three-Pointer Invariant (LC 206)**:
  At any step, `prev` anchors the reversed prefix, `curr` points to the active node being re-pointed, and `nxt` preserves the unvisited suffix. Each reassignment `curr.next = prev` operates in $\mathcal{O}(1)$ space.
...
#### 1. What is Head Insertion? (Prepending to Local Anchor)
In subsegment reversal (LC 92), moving pointers forward across the sublist severs the sublist from the rest of the chain, requiring 4 boundary pointers to reconnect later.
**Head Insertion (头插法)** avoids this by maintaining **fixed anchor positions**:
- **`prev` is static**: Anchored strictly at the node immediately before the reversal zone (`left - 1`);
- **`curr` is static in reference**: Always references the original first node of the subsegment. As subsequent nodes are peeled off and prepended to `prev`, `curr` is pushed progressively toward the end of the reversed subsegment;
- **`nxt` is dynamically prepended**: In each iteration, `curr.next` is extracted and inserted directly between `prev` and `prev.next`.

---

#### 2. Step-by-Step State Machine Walkthrough (head=[1,2,3,4,5], left=2, right=4)

```text
[Initial Setup] Position prev at 1, curr at 2. Total iterations = (right - left = 2):
  dummy -> [1]  ->  [2]  ->  [3]  ->  [4]  ->  [5] -> None
           prev     curr     nxt

[Iteration 1] Peel off node 3 and prepend after prev:
  1. Unlink:  nxt = curr.next          (lock node 3)
  2. Bypass:  curr.next = nxt.next      (node 2 skips 3 to point to 4: [2] -> [4])
  3. Prepend: nxt.next = prev.next      (node 3 points to current sub-head 2: [3] -> [2])
  4. Anchor:  prev.next = nxt           (node 1 connects to new sub-head 3: [1] -> [3])
  
  Resulting State:
  dummy -> [1]  ->  [3]  ->  [2]  ->  [4]  ->  [5] -> None
           prev              curr

[Iteration 2] Peel off node 4 and prepend after prev:
  1. Unlink:  nxt = curr.next          (lock node 4)
  2. Bypass:  curr.next = nxt.next      (node 2 skips 4 to point to 5: [2] -> [5])
  3. Prepend: nxt.next = prev.next      (node 4 points to current sub-head 3: [4] -> [3])
  4. Anchor:  prev.next = nxt           (node 1 connects to new sub-head 4: [1] -> [4])
  
  Resulting State:
  dummy -> [1]  ->  [4]  ->  [3]  ->  [2]  ->  [5] -> None
           prev                       curr

[Finish] Return dummy.next -> [1, 4, 3, 2, 5].
```
...
| Evaluation Metric | Two-Pointer Sliding (LC 206 Full Reversal) | Head Insertion (LC 92 Subsegment Reversal) |
| :--- | :--- | :--- |
| **Mental Model** | **"Turn and Advance Step-by-Step"** | **"Fixed Anchor, Peel and Prepend"** |
| **Pointer Roles** | `prev` (reversed head), `curr` (active node), `nxt` (suffix buffer) | `prev` (fixed anchor), `curr` (sublist tail anchor), `nxt` (moving card) |
| **Pointer Motion** | `prev` and `curr` **advance forward in lockstep** | `prev` and `curr` **never advance forward**; only targets change |
| **Loop Counter** | `while curr:` ($N$ times) | `for _ in range(right - left):` ($R - L$ times) |
| **4-Line Invariant** | 1. `nxt = curr.next`<br>2. `curr.next = prev`<br>3. `prev = curr`<br>4. `curr = nxt` | 1. `nxt = curr.next`<br>2. `curr.next = nxt.next`<br>3. `nxt.next = prev.next`<br>4. `prev.next = nxt` |
| **Optimal Use Case**| Complete in-place reversal of the entire list | Partial subsegment reversal, Reverse Nodes in k-Group |

**Symmetric Hand-off Rule for Head Insertion**:
```text
nxt       = curr.next
curr.next = nxt.next
nxt.next  = prev.next
prev.next = nxt
```
> The LHS of line $k$ cascades directly into the RHS of line $k+1$, forming an unbreakable four-step closed loop.
...
- **Time Complexity**:
  - LC 206 Full Reversal: $\mathcal{O}(N)$, visiting each node once.
  - LC 92 Subsegment Reversal: $\mathcal{O}(N)$ in a single pass up to `right`, executing exactly $R - L$ constant-time insertions.
- **Space Complexity**:
  - Iterative Two Pointers & Head Insertion: strictly $\mathcal{O}(1)$ auxiliary space.
  - Recursive Reversal: $\mathcal{O}(N)$ call stack depth.
