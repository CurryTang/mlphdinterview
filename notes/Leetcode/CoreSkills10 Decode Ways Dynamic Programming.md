...
#### Min Cost Climbing Stairs（使用最小花费爬楼梯 · LC 746）
...
#### House Robber（打家劫舍 I · 线性房屋 · LC 198）
- **题目描述**：你是一个专业的小偷，计划偷窃沿街的房屋。每间房内都藏有一定的现金，影响你偷窃的唯一制约因素就是相邻的房屋装有相互连通的防盗系统，如果两间相邻的房屋在同一晚上被小偷闯入，系统会自动报警。给定一个代表每个房屋存放金额的非负整数数组 `nums`，计算你在不触动报警装置的情况下，一夜之内能够偷窃到的最高金额。
- **约束**：两间相邻的房屋不能在同一晚上被盗。
...
#### House Robber II（打家劫舍 II · 环形房屋 · LC 213）
- **题目描述**：在打家劫舍 I 的基础上，街区里所有的房屋都**围成一圈**，这意味着第一间房屋和最后一间房屋是紧挨着的。同时，相邻的房屋装有相互连通的防盗系统。给定代表每个房屋存放金额的数组 `nums`，计算在不触发报警的前提下能偷窃到的最高金额。
...
### 4. 字符串前缀切分：Word Break（单词拆分 · LC 139）

- **题目描述**：给你一个字符串 $s$ 和一个字符串列表 `wordDict` 作为字典。请你判断是否可以利用字典中出现的单词拼接出 $s$（可以重复使用字典中的单词，不要求字典中的单词全部使用）。
...
### 5. 历史扫描：Longest Increasing Subsequence（LIS 最长递增子序列 · LC 300）
...
### 6. 符号翻转与极值双追踪：Maximum Product Subarray（乘积最大子数组 · LC 152）
...
#### 1. 对齐与公共子序列：Longest Common Subsequence（LCS · LC 1143）

- **题目描述**：给定两个字符串 `text1` 和 `text2`，返回这两个字符串的最长 **公共子序列** 的长度。如果不存在公共子序列，返回 0。一个字符串的子序列是指这样一个新的字符串：它是由原字符串在不改变字符相对顺序的情况下删除某些字符（也可以不删除任何字符）后组成的新字符串（例如，`"ace"` 是 `"abcde"` 的子序列，但 `"aec"` 不是）。
...
### 2. 三向决策：Edit Distance（编辑距离 · LC 72）
...
#### 原题背景与题意解构（Problem Context）

- **LeetCode 97 · Interleaving String**：
  给定三个字符串 $s_1, s_2, s_3$，请判断 $s_3$ 能否由 $s_1$ 和 $s_2$ **交错（Interleave）**拼接而成。
- **交错规则**：交错时必须**完全保留 $s_1$ 与 $s_2$ 各自内部字符的原有相对前后次序**。
- **长度先决守恒条件**：$s_3$ 中的每个字符必须恰好来自于 $s_1$ 或 $s_2$。若 $len(s_1) + len(s_2) \neq len(s_3)$，任何拼接都不可行，直接返回 `False`。

#### 全局宏观思维与解题心法（Overall Thoughts）

1. **为什么不能用贪心双指针？（面试核心陷阱）**：
   - 假设 $s_1 = \text{"aab"}, s_2 = \text{"aac"}, s_3 = \text{"aaabac"}$。
   - 当匹配第一个字符 `'a'` 时，$s_1[0]$ 与 $s_2[0]$ 均为 `'a'`。此时指针出现**分支抉择（Branching Decision）**：若贪心选择消耗 $s_1$，后续可能陷入无解；而选择消耗 $s_2$ 却可能成功。
   - 贪心策略无法预见未来的字符碰撞，一旦选错将无法自愈。因此必须使用具备分支回溯或全局状态记忆的算法（记忆化 DFS 或 DP）。
2. **几何网格寻路隐喻（物理直觉）**：
   - 将问题映射到一个 $(m+1) \times (n+1)$ 的二维网格中，起点为 $(0, 0)$，终点为 $(m, n)$。
   - **向下走一步** $(i-1, j) \to (i, j)$：代表当前步由 $s_1[i-1]$ 贡献，匹配 $s_3[i+j-1]$；
   - **向右走一步** $(i, j-1) \to (i, j)$：代表当前步由 $s_2[j-1]$ 贡献，匹配 $s_3[i+j-1]$；
   - 原题完全等价于：**在网格中寻找一条从左上角 $(0, 0)$ 连通到右下角 $(m, n)$ 的合法路径**！
3. **坐标守恒与无后效性**：
   - 当在 $s_1$ 中消耗了前 $i$ 个字符、在 $s_2$ 中消耗了前 $j$ 个字符时，在目标串 $s_3$ 中消耗的字符总数被数学**严格锁定为 $i+j$**，待匹配字符下标永远是 $s_3[i+j-1]$。无需第三维指针！

#### 状态定义与转移方程

- **状态定义**：$dp[i][j]$ 表示 $s_1[:i]$ 与 $s_2[:j]$ 能否交错组成 $s_3[:i+j]$。
- **状态转移**：当前字符 $s_3[i+j-1]$ 可以来自 $s_1[i-1]$（从上方转移）或 $s_2[j-1]$（从左方转移）：
...
class Solution:
    def isInterleave(self, s1: str, s2: str, s3: str) -> bool:
        ...
        # 空间压缩前：标准 (m+1) x (n+1) 二维布尔网格
        dp = [[False] * (n + 1) for _ in range(m + 1)]
        dp[0][0] = True  # 空串 s1 与空串 s2 天然匹配空串 s3
        
        # 第一列：完全不取 s2，只由 s1 向下走
        for i in range(1, m + 1):
            dp[i][0] = dp[i - 1][0] and s1[i - 1] == s3[i - 1]
            
        # 第一行：完全不取 s1，只由 s2 向右走
        for j in range(1, n + 1):
        ...
        for i in range(1, m + 1):
            for j in range(1, n + 1):
                dp[i][j] = (
                    (dp[i - 1][j] and s1[i - 1] == s3[i + j - 1])  # 从上方来 (消耗 s1[i-1])
                    or (dp[i][j - 1] and s2[j - 1] == s3[i + j - 1])  # 从左方来 (消耗 s2[j-1])
                )
        return dp[m][n]
```
...
由于计算新一行 $dp[i][j]$ 时，只依赖上一行的同列旧值（上方邻居 $dp[i-1][j]$）以及当前行已更新的前一列新值（左方邻居 $dp[i][j-1]$），状态天然可压缩为单行一维数组。
...
class Solution:
    def isInterleave(self, s1: str, s2: str, s3: str) -> bool:
        ...
        for i in range(1, m + 1):
            # 第 0 列 (j = 0) 仅依赖上方未覆盖的 dp[0] 与 s1[i-1]
            dp[0] = dp[0] and s1[i - 1] == s3[i - 1]
            for j in range(1, n + 1):
                # dp[j] (等号右侧未更新) 代表上方邻居；dp[j-1] (刚更新完) 代表左方邻居
...
- **复杂度**：时间 $O(mn)$，空间 $O(n)$（若 $m < n$，亦可互换 $s1$ 与 $s2$ 实现 $O(\min(m, n))$ 极致空间优化）。

---

### 4. 字符复用与子序列计数：Distinct Subsequences（不同的子序列 · LC 115）

- **题目描述**：给你两个字符串 `s` 和 `t`，统计并返回在 `s` 的 **子序列** 中 `t` 出现的个数。测试用例保证答案在 32 位带符号整数范围内。
...
### 5. 通配分支：Regular Expression Matching（正则表达式匹配 · LC 10）

- **题目描述**：给你一个字符串 `s` 和一个字符规律 `p`，请你来实现一个支持 `'.'` 和 `'*'` 的正则表达式匹配。`'.'` 匹配任意单个字符；`'*'` 匹配零个或多个前面的那一个元素。所谓匹配，是要涵盖 **整个** 字符串 `s` 的，而不是部分字符串。
...
### 1. 区间回文族：Longest Palindromic Substring & Palindromic Substrings（LC 5 & LC 647）

- **题目描述**：
  - **Palindromic Substrings (LC 647 · 回文子串)**：给你一个字符串 `s`，请你统计并返回这个字符串中 **回文子串** 的数目。具有不同开始位置或结束位置的子串，即使由相同的字符组成，也会被视作不同的子串。
  - **Longest Palindromic Substring (LC 5 · 最长回文子串)**：给你一个字符串 `s`，找到 `s` 中最长的回文子串。
...
### 2. 逆向思维之“最后戳破”：Burst Balloons（戳气球 · LC 312）

- **题目描述**：有 $n$ 个气球，编号为 $0$ 到 $n - 1$，每个气球上都标有一个数字，这些数字存在数组 `nums` 中。现在要求你戳破所有的气球。戳破第 $i$ 个气球可以获得 `nums[i - 1] * nums[i] * nums[i + 1]` 枚硬币。这里的 $i - 1$ 和 $i + 1$ 代表和 $i$ 相邻的两个气球的序号。如果 $i - 1$ 或 $i + 1$ 超出了数组的边界，那么就当它是一个数字为 $1$ 的气球。求所能获得硬币的最大数量。

#### 一、直觉陷阱：为什么正向思维（“先戳哪一个”）会彻底崩溃？

许多人面对本题的第一反应是贪心，或者正向分治递归：“在当前区间内，枚举先戳哪一个气球 $k$”。

然而，这会导致致命的**动态邻居粘连（Dynamic Boundary Entanglement）**问题：
...
- **子问题边界被动态摧毁**：当 $k$ 被戳破后，$k$ 左边的子问题（如气球 $B$）与 $k$ 右边的子问题（如气球 $C$）**直接发生了物理碰撞并相互粘连**！
- **无后效性彻底荡然无存**：如果接下来你打算在左边子问题中戳破 $B$，那么 $B$ 的右邻居直接穿透到了右边子问题中！此时计算戳破 $B$ 的金币收益，竟然取决于**右边子问题里到底还剩下谁活着**；反过来，右边戳气球的收益也取决于左边的残局。
- 两个子问题之间形成了死锁般的交叉依赖（Circular Inter-dependency），根本无法写出合法的分治状态转移方程！

---

#### 二、神来之笔：逆向思维 · 倒推“谁是最后一个被戳破的”

既然正着想“谁先死”会导致活下来的人邻居不断动态漂移，我们能不能**倒过来想**：
> **在开区间 $(i, j)$ 内的所有气球中，谁是这片区域内最后一个被戳破的终极幸存者？**

假设我们枚举气球 $k \in (i, j)$ 是区间 $(i, j)$ 内**最后一个被戳破**的气球，仔细审视此时的决战终局：

```text
逆向思维的时间线倒流图景：
阶段 1（前序战斗）：开区间 (i, k) 内部的气球已经在内部全军覆没（全部被戳光）！
阶段 2（前序战斗）：开区间 (k, j) 内部的气球也已经在内部全军覆没（全部被戳光）！
阶段 3（决战终局）：此时开区间 (i, j) 内寸草不生，只孤零零留下了最后一个气球 k！

       ┌──────────────┐                  ┌──────────────┐                  ┌──────────────┐
       │ 外部边界 a[i] │ <──────────────> │ 幸存气球 a[k] │ <──────────────> │ 外部边界 a[j] │
       └──────────────┘                  └──────────────┘                  └──────────────┘
      （不可侵犯的固定护栏）               （最后离开战场）               （不可侵犯的固定护栏）
```

- **确定性奇迹**：
  1. 因为开区间 $(i, k)$ 内的气球**全都没了**，气球 $k$ 的左边直接跨过了所有死者，其左邻居**必然且确定就是固定的外部边界 $a[i]$**！
  2. 因为开区间 $(k, j)$ 内的气球**全都没了**，气球 $k$ 的右边直接跨过了所有死者，其右邻居**必然且确定就是固定的外部边界 $a[j]$**！
  3. 戳破最后这个气球 $k$ 带来的金币收益，**是一笔没有任何歧义的固定死账**：
     $$\text{Coins}_k = a[i] \times a[k] \times a[j]$$
- **完全解耦与无后效性恢复**：
  - 在戳破 $k$ 之前，左边子问题 $(i, k)$ 无论怎么乱戳，其左右固定边界永远是 $a[i]$ 与 $a[k]$；
  - 右边子问题 $(k, j)$ 无论怎么乱戳，其左右固定边界永远是 $a[k]$ 与 $a[j]$；
  - 气球 $k$ 宛如一堵横亘在左右之间的巍峨高墙，使得 $(i, k)$ 与 $(k, j)$ **彼此完全物理隔离、互不影响、独立求解**！

---

#### 三、开区间 $(i, j)$ 与虚拟哨兵（Sentinels）的代数抽象

为了彻底消除数组下标越界的繁琐特判，我们在原数组两端各补一个数值为 `1` 的虚拟哨兵气球：
- 原数组 `nums` 长度为 $n$；
- 扩充数组 $a = [1] + nums + [1]$，总长度为 $N = n + 2$（下标范围 $0 \le i \le n + 1$）。

##### 为什么必须采用“开区间 $(i, j)$”？
- 开区间 $(i, j)$ 表示：**我们只戳破严格位于 $i$ 和 $j$ 之间的所有气球，而两端的边界哨兵 $a[i]$ 与 $a[j]$ 坚决不戳**（它们充当固定护栏）。
- **原问题求解目标**：把原数组 `nums` 所有的气球全戳破，对应开区间 $(0, n+1)$，最终答案即为：$dp[0][n+1]$。

##### 状态定义与转移方程
- **状态定义**：$dp[i][j]$ 表示将开区间 $(i, j)$ 内部的所有气球全部戳破所能获得的最大金币总数。
- **状态转移**：在开区间 $(i, j)$ 内枚举**最后一个戳破的气球 $k$**（$i < k < j$）：

$$dp[i][j] = \max_{i < k < j} \Bigl( \underbrace{dp[i][k]}_{\text{戳光左子区间}} + \underbrace{a[i] \cdot a[k] \cdot a[j]}_{\text{最后戳破 } k \text{ 的确定金币}} + \underbrace{dp[k][j]}_{\text{戳光右子区间}} \Bigr)$$

- **Base Cases**：
  - 当 $j - i \le 1$ 时（例如 $(0, 1)$ 或 $(i, i+1)$），开区间内部根本没有任何气球可戳，金币数为 0：
    $$dp[i][i+1] = 0$$

---

#### 四、经典算例步进手推（Step-by-Step Dry Run with `nums = [3, 1, 5, 8]`）

假设输入 `nums = [3, 1, 5, 8]`，添加两端哨兵后：
$$a = [1, 3, 1, 5, 8, 1], \quad N = 6 \quad (\text{下标 } 0 \sim 5)$$
我们的目标是求全局大开区间 $dp[0][5]$。

##### 第一阶段：跨度 $length = j - i = 2$（开区间内只有 1 个气球）
此时该气球就是唯一的幸存者，也是唯一的“最后一个戳破者”：
- $(0, 2) \to k=1$ (值 3): $dp[0][1] + a[0] \cdot a[1] \cdot a[2] + dp[1][2] = 0 + 1 \times 3 \times 1 + 0 = 3$
- $(1, 3) \to k=2$ (值 1): $dp[1][2] + a[1] \cdot a[2] \cdot a[3] + dp[2][3] = 0 + 3 \times 1 \times 5 + 0 = 15$
- $(2, 4) \to k=3$ (值 5): $dp[2][3] + a[2] \cdot a[3] \cdot a[4] + dp[3][4] = 0 + 1 \times 5 \times 8 + 0 = 40$
- $(3, 5) \to k=4$ (值 8): $dp[3][4] + a[3] \cdot a[4] \cdot a[5] + dp[4][5] = 0 + 5 \times 8 \times 1 + 0 = 40$

##### 第二阶段：跨度 $length = j - i = 3$（开区间内有 2 个气球，枚举 $k$）
- **求解 $(0, 3)$**，内部气球 $\{1, 2\}$（值 $3, 1$）：
  - 若 $k=1$ (值 3) 最后戳破：$dp[0][1] + a[0] \cdot a[1] \cdot a[3] + dp[1][3] = 0 + 1 \times 3 \times 5 + 15 = 30$
  - 若 $k=2$ (值 1) 最后戳破：$dp[0][2] + a[0] \cdot a[2] \cdot a[3] + dp[2][3] = 3 + 1 \times 1 \times 5 + 0 = 8$
  - 取极大值：$dp[0][3] = \max(30, 8) = 30$
- **求解 $(1, 4)$**，内部气球 $\{2, 3\}$（值 $1, 5$）：
  - 若 $k=2$ 最后戳破：$dp[1][2] + a[1] \cdot a[2] \cdot a[4] + dp[2][4] = 0 + 3 \times 1 \times 8 + 40 = 64$
  - 若 $k=3$ 最后戳破：$dp[1][3] + a[1] \cdot a[3] \cdot a[4] + dp[3][4] = 15 + 3 \times 5 \times 8 + 0 = 135$
  - 取极大值：$dp[1][4] = \max(64, 135) = 135$
- **求解 $(2, 5)$**，内部气球 $\{3, 4\}$（值 $5, 8$）：
  - 若 $k=3$ 最后戳破：$dp[2][3] + a[2] \cdot a[3] \cdot a[5] + dp[3][5] = 0 + 1 \times 5 \times 1 + 40 = 45$
  - 若 $k=4$ 最后戳破：$dp[2][4] + a[2] \cdot a[4] \cdot a[5] + dp[4][5] = 40 + 1 \times 8 \times 1 + 0 = 48$
  - 取极大值：$dp[2][5] = \max(45, 48) = 48$

##### 后续跨度归纳：
- $length = 4$：求出 $dp[0][4] = 159$ 与 $dp[1][5] = 164$；
- $length = 5$（全局终局 $dp[0][5]$）：
  - 枚举 $k \in \{1, 2, 3, 4\}$，当 $k=1$ 或 $k=4$ 时均可得到全局最优值：$167$！

---

#### 五、填表循环拓扑序与双重实现方案

计算 $dp[i][j]$ 时，依赖的子状态 $dp[i][k]$ 和 $dp[k][j]$ 满足：
$$(k - i) < (j - i) \quad \text{且} \quad (j - k) < (j - i)$$
即**依赖的子区间跨度严格小于当前区间跨度**。因此有两种标准迭代顺序：

##### 方案一：按开区间跨度 $length$ 递增（最直观、面试推荐）
...
class Solution:
    def maxCoins(self, nums: list[int]) -> int:
        ...
        # 2. 按开区间跨度 length = j - i 递增遍历 (跨度从 2 到 n - 1)
        for length in range(2, n):
            for i in range(0, n - length):
                j = i + length
                # 枚举区间 (i, j) 中最后被戳破的气球 k
...
- **复杂度分析**：
  - **时间复杂度**：状态总数有 $O(n^2)$ 个，每个状态需要枚举 $k$（转移耗时 $O(n)$），总时间复杂度为 $O(n^3)$。
  - **空间复杂度**：需要开辟一个 $(n+2) \times (n+2)$ 的二维 DP 矩阵，空间复杂度为 $O(n^2)$。
...
> 💡 **戳气球三步心法**：
> 1. **两端补一化开间**：首尾添 `1` 作哨兵，$(0, n+1)$ 锁死全场边界。
> 2. **倒推末位解死锁**：正向先戳邻居飘忽不定（无后效性崩溃）；逆向后戳前序尽归尘土，唯余 $a[i] \cdot a[k] \cdot a[j]$ 稳如泰山！
> 3. **跨度递增填方阵**：小开区间筑牢基石，大开区间左右开花。
...
### 2. 0/1 背包例题一：Partition Equal Subset Sum（分割等和子集 · 可行性 · LC 416）
...
### 3. 0/1 背包例题二：Target Sum（目标和 · 代数转化与计数 · LC 494）

- **题目描述**：给你一个非负整数数组 `nums` 和一个整数 `target`。向数组中的每个整数前添加 `'+'` 或 `'-'`，然后串联起所有整数，可以构造一个**表达式**。返回可以通过上述方法构造的、运算结果等于 `target` 的不同**表达式**的数目。
...
### 4. 完全背包例题一：Coin Change（零钱兑换 · 最少枚数 · LC 322）
...
### 5. 完全背包例题二：Coin Change II（零钱兑换 II · 组合数 vs 排列数 · LC 518）

- **题目描述**：给你一个整数数组 `coins` 表示不同面额的硬币，另给一个整数 `amount` 表示总金额。请你计算并返回可以凑成总金额的硬币**组合数**。如果任何硬币组合都不能凑出总金额，返回 `0`。假设每一种面额的硬币有无限个。
- **核心陷阱：组合数 vs 排列数**：
...
### 6. 多维费用 0/1 背包：Ones and Zeroes（一和零 · 二维容量约束 · LC 474）

- **题目描述**：给你一个二进制字符串数组 `strs` 和两个整数 `m` 和 `n`。请你找出并返回 `strs` 的最大子集的长度，该子集中**最多**有 `m` 个 `'0'` 和 `n` 个 `'1'`。如果 `x` 的所有元素也是 `y` 的元素，集合 `x` 是集合 `y` 的**子集**。
...
### 1. 有序网格扫掠：Unique Paths（不同路径 · LC 62）

- **题目描述**：一个机器人位于一个 $m \times n$ 网格的左上角（起始点为 $grid[0][0]$）。机器人每次只能向下或者向右移动一步。机器人试图达到网格的右下角（即 $grid[m - 1][n - 1]$）。问总共有多少条不同的路径？
- **规则**：只能向下或向右走 $\implies$ 拓扑依赖天然有序（只依赖上方与左方）。
- **状态定义**：$dp[i][j]$ 表示从起点 $(0, 0)$ 到达格子 $(i, j)$ 的不同路径数。
- **转移方程**：$dp[i][j] = dp[i-1][j] + dp[i][j-1]$。
- **边界条件**：第一行 $dp[0][j] = 1$（只能一直向右），第一列 $dp[i][0] = 1$（只能一直向下）。

#### 空间压缩前：标准二维 DP 表（清晰直观）
...
由于计算 $dp[i][j]$ 时只依赖上方（同列上一行未更新值 $dp[i-1][j]$）与左方（同行前一列刚更新值 $dp[i][j-1]$），行与行之间可直接复用同一个长度为 $n$ 的单行数组。
...
- **复杂度**：时间 $O(mn)$，空间 $O(n)$。
- **边界特性**：$m=1$ 或 $n=1$ 时循环不执行，直接返回 `dp[-1] = 1`，天然覆盖极端用例。

---

### 2. 无序网格与隐式 DAG：Longest Increasing Path in a Matrix（矩阵中的最长递增路径 · LC 329）

- **题目描述**：给定一个 $m \times n$ 整数矩阵 `matrix`，找出其中**最长严格递增路径**的长度。对于每个单元格，你可以往上、下、左、右四个方向移动。你**不能**在对角线方向移动或移动到边界外（即不能环绕）。
...
### 经典实战：Best Time to Buy and Sell Stock with Cooldown（买卖股票含冷冻期 · LC 309）

- **题目描述**：给定一个整数数组 `prices`，其中 `prices[i]` 表示第 $i$ 天的股票价格。设计一个算法计算出最大利润。你可以尽可能多地完成交易，但有以下约束：卖出股票后，你无法在第二天买入股票（即冷冻期为 1 天）。注意：你不能同时参与多笔交易（必须在再次购买前出售掉之前的股票）。
...
### 1. Kadane's Algorithm（最大子数组和 · 负资产切除 · LC 53）
...
### 2. Jump Game（跳跃游戏 · 最左好位置压缩 · LC 55）
