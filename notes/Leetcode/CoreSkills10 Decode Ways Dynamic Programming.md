...
背包问题是一类经典的带约束优化与计数问题。其最核心的分类与循环方向区别如下：

```text
背包分类核心判定：
┌─────────────────┬───────────────────┬───────────────────────────────────────┐
│ 背包类型        │ 物品使用次数      │ 一维压缩内层容量遍历方向              │
├─────────────────┼───────────────────┼───────────────────────────────────────┤
│ 0/1 背包        │ 每个物品最多选 1 次│ 倒序 (Capacity down to Weight) ➔ 防复选│
│ 完全背包        │ 每个物品可无限复选 │ 正序 (Weight up to Capacity) ➔ 允复选 │
└─────────────────┴───────────────────┴───────────────────────────────────────┘
```

---

### 1. 0/1 背包例题一：Partition Equal Subset Sum（分割等和子集 · 可行性）
...
### 4. 完全背包例题二：Coin Change II（零钱兑换 II · 组合数 vs 排列数）
...
| 目标类型 | `dp[0]` 初始值 | 其余 `dp[1..W]` 初始值 | 转移算子 |
|---|---|---|---|
| 最大价值（允许不装满） | `0` | `0` | $\max(dp[j], dp[j-w] + v)$ |
| 最大价值（必须恰好装满）| `0` | `-inf` | $\max(dp[j], dp[j-w] + v)$ |
| 最小数量（必须恰好装满）| `0` | `+inf` (或 `amount + 1`) | $\min(dp[j], dp[j-c] + 1)$ |
| 方案数计数 | `1` | `0` | $dp[j] += dp[j-w]$ |
| 可行性判定 | `True` | `False` | $dp[j] = dp[j] \lor dp[j-w]$ |
