...
### 2D DP 矩阵填表与西南角依赖原理（State Transition Matrix）

1. **状态定义**：
   - `$dp[i][j]$` 是一个布尔值，表示闭区间子串 `$s[i..j]$` 是否为回文串。
   - 由于只有 `$i \le j$` 的子串有物理意义，因此整个二维矩阵**只有主对角线及右上三角区有效**（左下三角 `$i > j$` 为无效区域）。

2. **状态转移三部曲**：
   - **Step 1：两端字符匹配检查**
     如果 `$s[i] \neq s[j]$`，首尾不同，必定不是回文，直接判定 `$dp[i][j] = False$`；
   - **Step 2：短子串直接定论（Base Cases，长度 $\le 2$）**
     若 `$s[i] == s[j]$`：
     - 若 `$j - i = 0$`（长度为 1，即对角线 `$i = j$`）：单字符自身天然回文，`$dp[i][i] = True$`；
     - 若 `$j - i = 1$`（长度为 2，如 `"aa"`）：相邻两字符相同即为回文，`$dp[i][i+1] = True$`；
   - **Step 3：长子串剥洋葱（Length $\ge 3$）**
     若 `$s[i] == s[j]$` 且 `$j - i \ge 2$`：
     去除两端后的内层子串为 `$s[i+1..j-1]$`。此时只需查询矩阵中**行号加 1（向下挪一行）、列号减 1（向左挪一列）**的格子，即**西南角邻居** `$dp[i+1][j-1]$`：
     $$dp[i][j] = dp[i+1][j-1]$$

3. **填表循环顺序的本质**：
   由于计算当前格子 `(i, j)` 需要其左下方邻居 `(i+1, j-1)` 的值，必须保证**更短的子区间先被计算出来**。代码中采用：
   - 外层循环：`$i$` 从 `$n-1$` 递减到 `0`（从底向上）；
   - 内层循环：`$j$` 从 `$i$` 递增到 `$n-1$`（从左向右）。
   或者外层枚举区间长度 `$L = 1..n$`，内层枚举起点 `$i$`。
