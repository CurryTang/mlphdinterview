...
题目与参考答案：[[BusinessAlgorithm06 Quick Coding.md#QC02 构建倒排索引|QC02 构建倒排索引]]。
...
BM25 年纪不小，但面试里仍值得讲。它快、稳、可解释，也是稠密召回和生成式检索必须超过的基线。
...
题目与参考答案：[[BusinessAlgorithm06 Quick Coding.md#QC03 ItemCF 推荐|QC03 ItemCF 推荐]]。
...
内容召回对新品和长尾尤其重要。协同信号尚未形成时，内容是少数可用信息。它的风险是"看起来相似"不等于"同一用户会喜欢"，因此常与协同召回并用。
...
实用做法包括固定或动态配额、通道内排序后截断、分数归一化、学习融合、去重后回填。更重要的是做归因：记录每个候选由哪些通道召回，评估单路覆盖、重叠率、最终曝光和转化。
...
题目与参考答案：[[BusinessAlgorithm06 Quick Coding.md#QC04 Reciprocal Rank Fusion|QC04 Reciprocal Rank Fusion]]。
...
这比直接重训双塔有效得多。很多"召回问题"其实是过滤或配额问题。
