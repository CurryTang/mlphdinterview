...
| **个性化精细排序** | **Group AUC (GAUC)** | 在每个用户 $u$ 内部单独计算 $	ext{AUC}_u$，然后按曝光量或点击量加权平均：<br>$$	ext{GAUC} = rac{\sum_{u} w_u \cdot 	ext{AUC}_u}{\sum_u w_u}$$ | **工业界精排离线核心北极星**！剥离了用户间静态偏置，纯粹衡量模型在“单个用户展示列表内部的相对排序优劣”。 |
| **概率打分质量** | **LogLoss (交叉熵)** | 衡量预测连续概率与真实 0/1 标签的对数损失。 | 直接反映概率拟合质量；对绝对数值极其敏感，受负采样影响大。 |
| **位置敏感度** | **NDCG@K / MRR** | 针对 Top-K 推荐列表，引入对数位置衰减因子 $rac{1}{\log_2(	ext{rank}+1)}$。 | 贴合用户视觉只看首屏（Top-3 / Top-5）的真实浏览习惯。 |
...
$$	ext{PCOC} = rac{\sum_{i=1}^N \hat{p}_i}{\sum_{i=1}^N y_i} = rac{	ext{模型全样本预估平均概率}}{	ext{真实观测平均点击率}}$$
...
将预测概率分装到 10~20 个等频分桶（Bins）中，横轴绘制每个桶的平均预估概率 $ar{\hat{p}}$，纵轴绘制该桶的实际经验转化率 $ar{y}$。若曲线严格贴合 $y = x$ 45度对角线，则说明在不同置信度区间均具备优良校准性。
...
2. **样本规模与采样（Sample Size & Sampling）**：
   “训练集基于近 30 天的百亿级真实用户曝光日志构建，并配合线上小时级流式增量微调。在样本构建上，**精排与召回存在本质差异**：精排只采用真实的**曝光未点击（Impression-Unclicked）作为负样本**。针对极端正负不平衡，我们采用负样本随机下采样（保留率 $w = 10\%$），并在推理时通过校准公式 $p = rac{\hat{p}}{\hat{p} + (1-\hat{p})/w}$ 还原真实自然概率。”
