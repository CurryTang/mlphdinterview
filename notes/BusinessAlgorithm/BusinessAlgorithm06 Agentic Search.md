...
切块产物需要像索引一样可版本化。每个 chunk 保存 `document_id`、`chunk_id`、父节点、字符偏移、内容 hash、`source_version` 和 `acl_version`。正文可按标题切到 300 至 800 token；表格优先保留表头与完整行；代码按函数或类边界切；FAQ 则让问题和答案留在同一块。overlap 只用于补边界，不应让同一句话以五个近重复 chunk 占满 top-k。

块大小不是单独优化的超参数。离线同时扫 chunk policy、召回 top-k 和父块扩展策略，观察 claim recall、context precision、最终上下文 token 和答案支持率。小块召回后扩父块，常比直接检索大块更容易兼顾定位与完整证据；若父块扩展让无关内容激增，就只补命中句附近的兄弟块。

文档更新时先生成新版本 chunk 和索引，再原子切换可见版本。查询不能混到同一文档的新旧段落。删除和权限收紧要走更快的失效通道，并清理对应缓存；等下一次全量建库再处理，会留下越权或过期证据。
...
认证用户与租户
  -> 解析当前 ACL
  -> 在召回前过滤可见文档
  -> 用 principal + ACL version + index version 构造缓存键
  -> 把检索文本标记为不可信数据
  -> 生成并校验引用
  -> 需要工具操作时再做参数校验与确认
```

先召回再在生成前删越权文档并不安全。文档内容、标题和 embedding 都可能已经进入日志、缓存或 reranker。共享缓存若只按 query 建键，也会把高权限用户的结果返回给低权限用户。

检测到 prompt injection 时，默认动作是丢弃该 chunk 并用剩余证据重试。若关键 claim 只依赖这块证据，则拒答或返回普通搜索结果，而不是让模型自行判断那段指令是否可信。检索阶段默认不开放写操作工具；需要发邮件、修改文件或执行交易时，工具参数必须来自结构化状态，并在调用前重新做权限检查。日志保留命中的文档版本、被拦截规则和最终工具参数，便于复现安全事件。
