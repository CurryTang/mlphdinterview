...
两者之间还有 late interaction。以 ColBERT 类方法为例，query 与文档分别编码成 token 向量，文档 token 可预计算；在线再让每个 query token 在文档 token 中找最相似匹配并聚合：
...
它保留了词粒度匹配，比单向量双塔更细；存储和在线匹配又比 Cross-BERT 重。是否值得取决于候选规模、token 索引内存和延迟，而不是简单地把它称为"效果与成本折中"。
...
```text
固定候选集离线切片
  -> Side-by-Side / GSB 看整页 bad case
  -> shadow 比较分数、超时和缓存命中
  -> canary 小流量检查相关性档位与延迟护栏
  -> A/B 观察搜索成功与主动换词
```

监控不能只留一个总体 NDCG。至少按头部/长尾、实体、属性、时效和语言切片看分布漂移，并记录 tokenizer、模型、文档和缓存版本。若新模型超时、相关性零档进入头部或某类 query 空结果突增，应能独立切回旧相关性服务，不必回滚整条搜索链路。
