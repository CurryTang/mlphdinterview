...
其中 $Z(x) = \sum_y \pi_{\text{ref}}(y \mid x) \exp\left( \frac{1}{\beta} r(x, y) \r\right)$ 为配分函数（Partition Function）。
...
$$\nabla_\theta \mathcal{L}_{\text{DPO}}(\theta) = -\beta \mathbb{E} \left[ \underbrace{\sigma\left( \hat{r}_\theta(x, y_l) - \hat{r}_\theta(x, y_w) \r\right)}_{\text{动态权重系数 } w(x, y_w, y_l)} \cdot \left( \nabla_\theta \log \pi_\theta(y_w \mid x) - \nabla_\theta \log \pi_\theta(y_l \mid x) \r\right) \r\right]$$
...
| **SimPO** | 使用生成长度归一化的平均 Logits 差，结合 Target Margin 惩罚 | $-\log \sigma\left(\frac{\beta}{|y_w|}\log \pi_\theta(y_w) - \frac{\beta}{|y_l|}\log \pi_\theta(y_l) - \gamma \r\right)$ | **无需 (1 模型)** | **当前开源评测 SOTA**，彻底摆脱参考模型，且内生性消除长度偏见 |
